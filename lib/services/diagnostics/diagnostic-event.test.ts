...
describe("toDiagnosticEvent", () => {
  ...
  test("falls back to defaults when fields are wrong-typed or missing", () => {
    const event = toDiagnosticEvent({ seq: "nope", ts: null, type: 42 })
  ...
  test("treats a non-object JSON payload as unparsed", () => {
    const event = toDiagnosticEvent({ payload: "[1,2,3]" })
  ...
  test("keeps the raw payload but no parse for malformed JSON", () => {
    const event = toDiagnosticEvent({ payload: "{not json" })
...
describe("toDiagnosticConnectionError", () => {
  test("narrows a well-formed connection row", () => {
    const error = toDiagnosticConnectionError({
      seq: 3,
      ts: 1700000000000,
      type: "slack",
      status: "auth-failed",
  ...
  test("nulls an empty detail string", () => {
    const error = toDiagnosticConnectionError({ status: "error", detail: "" })
