...
export class FunnelListenersClient {
  ...
  async start(name: string): Promise<ListenerOpResult> {
    if (!this.isDaemonRunning()) return OFFLINE;

    return await this.call("POST", `/listeners/${encodeURIComponent(name)}/start`);
  }

  async stop(name: string): Promise<ListenerOpResult> {
    if (!this.isDaemonRunning()) return OFFLINE;

    return await this.call("DELETE", `/listeners/${encodeURIComponent(name)}`);
  }

  async restart(name: string): Promise<ListenerOpResult> {
    if (!this.isDaemonRunning()) return OFFLINE;

    return await this.call("POST", `/listeners/${encodeURIComponent(name)}/restart`);
  }
