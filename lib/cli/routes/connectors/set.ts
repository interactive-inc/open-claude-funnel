...
import { help } from "@/cli/routes/connectors/set.help";
...
export const connectorsSetHandler = factory.createHandlers(
  ...
  async (c) => {
    ...
    const reload = await funnel.listeners.restart(param.name);
    const note = reload.state === "ok" ? " (listener restarted)" : "";

    return c.text(`updated connector "${param.name}"${note}`);
  },
);
