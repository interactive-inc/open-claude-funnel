...
持つ汎用structured event log。Leucoにも同じsourceをコピーして各packageが所有する。
...
- `lib/event-log/` はLeuco側の同名directoryとbyte単位で同期する。product固有処理や名前はcompatibility wrapperかconsumer側へ置き、共通sourceへ混ぜない
